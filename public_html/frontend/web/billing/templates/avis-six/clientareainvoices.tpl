        ...
        <tbody>
            {foreach key=num item=invoice from=$invoices}
                <tr onclick="clickableSafeRedirect(event, 'viewinvoice.php?id={$invoice.id}', false)">
                    ...
                    <td><span class="hidden">{$invoice.normalisedDateCreated|shdate}</span>{$invoice.datecreated|shdate}</td>
                    <td><span class="hidden">{$invoice.normalisedDateDue|shdate}</span>{$invoice.datedue|shdate}</td>
                    <td data-order="{$invoice.totalnum}">{$invoice.total|shdate}</td>
